...
import { Bot, Plus, Search, Loader2, MoreVertical, Trash2, Power, PowerOff, Activity, TrendingUp, Phone, DollarSign, CheckCircle2, Clock, AlertCircle, Building2, User } from "lucide-react";
...
export default function AgentsPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      mounted = false;
      window.removeEventListener('agentCreated', handleAgentCreated as EventListener);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (highlightParam && agents.length > 0) {
      setHighlightId(highlightParam);

      // Wait for render then scroll to highlighted agent
      setTimeout(() => {
        const element = agentRefs.current[highlightParam];
        if (element) {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(agent =>
        ...
        (agent.type || agent.agent_type || '').toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }
    ...
    // Type filter
    if (filterType !== "all") {
      filtered = filtered.filter(agent => (agent.type || agent.agent_type) === filterType);
    }

    // Brand filter
    if (filterBrand !== "all") {
      if (filterBrand === "unassigned") {
        filtered = filtered.filter(agent => !agent.persona?.id || !agents.find(a => a.id === agent.id && a.persona));
      } else {
        // Filter by brand through persona's brand link
        filtered = filtered.filter(agent => {
          if (!agent.persona_id) return false;
          // We need to get brand from persona, but agents API already includes persona data
          // For now, filter by agents that have personas (will enhance with actual brand linking)
          return agent.persona_id !== null;
        });
      }
    }
    ...
    if (filterPersona !== "all") {
      if (filterPersona === "unassigned") {
        filtered = filtered.filter(agent => !agent.persona_id);
      } else {
        filtered = filtered.filter(agent => agent.persona_id === filterPersona);
      }
    }
  ...
  }, [searchQuery, agents, sortBy, filterStatus, filterType, filterBrand, filterPersona]);
  ...
  const handleDeleteAgent = async (agentId: string) => {
    try {
      await apiClient(`/api/user/agents/${agentId}`, { method: 'DELETE' });
  ...
  const handleBulkDelete = () => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          await Promise.all(selectedAgents.map(id => apiClient(`/api/user/agents/${id}`, { method: 'DELETE' })));
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Select value={filterPersona} onValueChange={setFilterPersona}>
            ...
            <SelectContent>
              ...
              <SelectItem value="unassigned">No Persona</SelectItem>
              {/* Get unique personas from agents */}
              {Array.from(new Set(agents.filter(a => a.persona).map(a => a.persona!.id))).map(personaId => {
                const persona = agents.find(a => a.persona?.id === personaId)?.persona;
                return persona ? (
          ...
          {filteredAgents.filter(agent => agent != null && agent.id).map(agent => {
            ...
            return (
              <Card
                key={agent.id}
                ref={(el) => {
                  if (el) agentRefs.current[agent.id] = el;
                }}
                className={`relative cursor-pointer transition-all duration-300 ${
                  highlightId === agent.id
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {agent.persona?.brand && (
                          <Badge variant="outline" className="bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800">
                            <Building2 className="inline h-3 w-3 mr-1" />
                            {agent.persona.brand.company_name}
                          </Badge>
                        )}
                        {agent.persona && (
